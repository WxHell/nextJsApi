...
export default function Page() {
  ...
  if (loading || !slugNews) return null;
        ...
        <ul className="text-black text-sm flex flex-row items-stretch h-full">
          {slugNews.map((item, index) => (
            index % 4 === 3 ? (
              <li
                key={`reklam-${index}`}
                className="relative border-r border-gray-300 px-4 min-w-[280px] max-w-[280px] flex flex-col justify-center px-4"
              >
              ...
              <li key={`news-${item._id ?? index}`} className="h-full">
                <NewsCard
                  id={item._id}
                  title={item.title}
