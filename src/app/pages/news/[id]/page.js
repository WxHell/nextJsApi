            ...
            <div className="w-[480px] bg-white border-b md:border-r border-gray-300 md:border-b-0 overflow-x-auto md:overflow-y-auto md:h-screen">
                <ul className="flex flex-row text-black text-sm w-full whitespace-nowrap md:flex-col">
                    {sortedNewsItems.map((item) => (
                        <li
                            key={item}
                            className={`relative border-b border-gray-300 p-6 flex flex-row items-start transition duration-300 ${selectedItem === item ? 'bg-yellow-50' : ''
                                }`}
                            onClick={() => setSelectedItem(item)}
                        >
                            <div className="absolute top-0 left-6 font-bold text-2xl pt-0 md:text-5xl opacity-25">{item}</div>
                            <img
                                className="mr-7 pt-3"
                                src={newsImages[item]}
                                alt={`Haber ${item}`}
