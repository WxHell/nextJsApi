...
export default function Home() {
  ...
  // Son geçerli haber listesini saklamak için state
  const [lastAllNews, setLastAllNews] = useState(null);

  // Yeni haberler gelirse lastAllNews'u güncelle
  useEffect(() => {
  ...
  // Gösterilecek haberler: yeni varsa onu, yoksa eskiyi göster
  const displayAllNews = news && news.length > 0 ? news : lastAllNews;

  // Hata ve yüklenme kontrolleri
        ...
        <div className="px-15">
          ...
          <div className="flex flex-row gap-4 items-center">
            <div className="flex flex-col gap-1.5">
              {[...Array(6)].map((_, i) => (
                <div key={i} className="h-[0.25px] w-9 bg-white"></div>
              ))}
            </div>
